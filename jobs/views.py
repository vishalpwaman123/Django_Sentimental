...
def audio(request):
	...
	if request.method == 'POST':  # Recognize POST method
		...
		for m in mp4_files:  # Looping Start
			print("Flag3")
			file, extension = os.path.splitext(m)  # Split File Name & Extension
			# Create Object Using Path With Audio File
			onair = sr.AudioFile('media/'+upload.name)
			try:
				origAudio = wave.open('media/'+upload.name, 'r')  # Open File And Read File
			...
			print("")
			clip_len = 7
			n_clips = math.floor(song_length/clip_len)  # Song Divide Number Of Part
			...
			with onair as source:
				while(n_clips >= 0):
					try:
						audio = r.record(source, duration=clip_len)  # Record Audio
						try:
...
def login(request):
	if request.method == 'POST':
		user = auth.authenticate(username = request.POST.get('username1'), password = request.POST.get('password'))
		if user is not None:
			auth.login(request,user)
